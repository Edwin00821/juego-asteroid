...
const Game = () => {
  ...
  const [UsersScore, SetUsersScore] = useState([{ user: "", score: null }]);
  ...
  const setShipPosition = (x, y) => {
    setGameState((prevState) => ({
      ...prevState,
      ship: { ...prevState.ship, pos: { x: x, y: y } },
    }));
  };
    ...
    if (!showStartModal) {
      setGameState((prevState) => {
        if (!prevState.gameOver) {
          ...
          const velocityChange = 0.05 * prevState.level;

          const newAngle =
            kp.l && !kp.r ? -shipRotation : kp.r && !kp.l ? shipRotation : 0;
          const newSpeed = kp.u ? -velocityChange : 0;

          const bA = prevState.ship.angle + newAngle;

          const velX =
            prevState.ship.vel.x - newSpeed * -Math.sin((Math.PI / 180) * bA);
          const velY =
            prevState.ship.vel.y - newSpeed * Math.cos((Math.PI / 180) * bA);
          ...
          if (kp.s && largestBulletLife < 110) {
            const bVelX =
              prevState.ship.vel.x + 5 * -Math.sin((Math.PI / 180) * bA);
            const bVelY =
              prevState.ship.vel.y + 5 * Math.cos((Math.PI / 180) * bA);
              ...
              if (
                ...
                switch (a.type) {
                  ...
                  case "S":
                    ...
                    break;
                  default:
                    console.error("unexpected asteroid type");
                }
              }
            });
          });
  ...
  const SetUsers = (userSocket) => {
    let userValid = false;
    UsersScore.forEach((userInState) => {
      if (validateUser(userSocket.user, userInState.user)) userValid = true;
    });
  ...
  return (
    <>
      {isMultiplayer && (
        <Users>
          Score:
          {UsersScore.sort((firstUser, secondUser) => {
      ...
      <form>
        {showStartModal && (
          <Start start={startGame} startMltiplayer={startMultiplayer} />
        )}
